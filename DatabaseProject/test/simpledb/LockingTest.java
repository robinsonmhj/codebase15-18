...
public class LockingTest extends TestUtil.CreateHeapFile {
  ...
  @Before public void setUp() throws Exception {
    ...
    for (int i = 0; i < 1025; ++i) {
      //int pgNo=i/bp.PAGE_SIZE;
      empty.insertTuple(tid, Utility.getHeapTuple(i, 2,empty.getId()));
    }
    ...
    bp.flushAllPages();
    //release the lock as it obtains the lock, I fix the bug for the code robin
    bp.releasePage(tid, p0);
    bp.releasePage(tid, p1);
    bp.releasePage(tid, p2);
    //bp = Database.resetBufferPool(BufferPool.DEFAULT_PAGES);
  }
   ...
   *   false otherwise
   */
  public void metaLockTester(TransactionId tid1, PageId pid1, Permissions perm1,TransactionId tid2, PageId pid2, Permissions perm2,boolean expected) throws Exception {
  ...
  public void grabLock(TransactionId tid, PageId pid, Permissions perm,boolean expected) throws Exception {
    ...
    // this is a bug, robin changes. If can get the lock, the errro should null, otherwise it is not null
    if(expected)
