...
public class HeapPageWriteTest extends SimpleDbTestBase {
    ...
    @Test public void addTuple() throws Exception {
        ...
        for (int i = 0; i < free; ++i) {
            Tuple addition = Utility.getHeapTuple(i, 2);//it reset the page id in the code
            //System.out.println("+++++Tuple to be insert:"+addition+",page is "+addition.getRecordId().getPageId());
            ...
            while (it.hasNext()) {
                ...
                if (addition.equals(tup)) {
                    found = true;
                    //System.out.println("addition"+addition+",id="+addition.getRecordId());
                    //System.out.println("tup"+tup+",id+"+tup.getRecordId());
                    // verify that the RecordId is sane
                    //System.out.println("===pageId==="+page.getId());
                    //assertEquals(page.getId(), tup.getRecordId().getPageId());//it will fail forever
                    assertFalse(page.getId().equals(tup.getRecordId().getPageId()));
                    break;
                }
            }
            assertTrue(found);
