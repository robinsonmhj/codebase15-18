...
/*    @Test public void testDoNotEvictDirtyPages()
        ...
        // A second transaction must find the tuple, as it is written to the disk
        t = new Transaction();
        t.start();
    ...
    @Test public void testDoNotEvictDirtyPages2()
            throws IOException, DbException, TransactionAbortedException {
        // Allocate a file with ~10 pages of data
        HeapFile f = SystemTestUtil.createRandomHeapFile(2, 512*10, null, null);
        Database.resetBufferPool(2);
        ...
        // A second transaction must not find the tuple, as it is aborted
        t = new Transaction();
        t.start();
        found = EvictionTest.findMagicTuple(f, t);
        assertFalse(found);
