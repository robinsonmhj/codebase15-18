...
public class LogTest extends SimpleDbTestBase {
    ...
    void insertRow(HeapFile hf, Transaction t, int v1, int v2)
        ...
        PageId pid=new HeapPageId(hf.getId(),0);
        value.setRecordId(new RecordId(pid,0));
    ...
    /* @Test public void PatchTest()
        ...
        doInsert(hf1, 1, 2);
        int count=Database.getLogFile().getTotalRecords();
        ...
        byte[] b1=p1.getPageData();
        for(int i=0;i<b.length;i++){
        	if(b[i]!=b1[i]){
        		System.out.println("b["+i+"="+b[i]);
        		System.out.println("b1["+i+"="+b1[i]);
        	}else{
        		//System.out.println("They are equals"+b[i]);
        		//System.out.println("b1["+i+"="+b1[i]);
        	}
        }
...
/*     
      @Test public void TestFlushAll()
        ...
        //System.out.println("xp1="+xp1+",xp2="+xp1);
        if(xp1.getNumEmptySlots() == xp2.getNumEmptySlots())
            throw new RuntimeException("LogTest: flushAllPages() had no effect");
        //relase all the locks, otherwise, the other test will never get the lock
        Database.getBufferPool().releasePage(t1.getId(), xp1.getId());
        Database.getBufferPool().releasePage(t1.getId(), xp2.getId());
    }*/
  ...
  /*  @Test public void TestAbort()
    ...
    */
    ...
    /*@Test public void TestAbortCommitInterleaved()
        ...
        abort(t1);
          //it is used to test no overlapped transactions		
//        Transaction t1 = new Transaction();
//        t1.start();
//        insertRow(hf1, t1, 3, 0);
//        insertRow(hf1, t1, 4, 0);
