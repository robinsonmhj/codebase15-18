...
public class EvictionTest extends SimpleDbTestBase {
    ...
    public static void insertRow(HeapFile f, Transaction t) throws DbException,
        ...
        //fix the bug that recordId is null robin
        value.setRecordId(new RecordId(new HeapPageId(f.getId(),f.numPages()+1),1));
        TupleIterator insertRow = new TupleIterator(Utility.getTupleDesc(2), Arrays.asList(new Tuple[]{value}));
        
 /*       insertRow.open();
        int i=0;
        while(insertRow.hasNext()){
        	System.out.println("+++++++++++"+i+",++++"+insertRow.next());
        }*/
        ...
        //System.out.println("====== insert should not have next "+insert.hasNext());
        //System.out.println("====== insert next is "+insert.next());
