...
public class TestUtil {
    ...
    public static boolean compareTuples(Tuple t1, Tuple t2) {
        ...
        for (int i = 0; i < t1.getTupleDesc().numFields(); ++i) {
            if (!(t1.getTupleDesc().getFieldType(i).equals(t2.getTupleDesc().getFieldType(i))))
                return false;
            //fix the bug, different class cannot be compared, AggregateTest line 132
            if (!(t1.getField(i).getClass().isInstance(t2.getField(i)))||!(t1.getField(i).equals(t2.getField(i))))
                return false;
        }
    ...
    public static void matchAllTuples(DbIterator expected, DbIterator actual) throws
        ...
        while (expected.hasNext()) {
            ...
            matched = false;
            actual.rewind();
            //there must be a bug. as actual go to the end and there is no match, if (!matched) will return true and thow exception
            boolean end=true;
