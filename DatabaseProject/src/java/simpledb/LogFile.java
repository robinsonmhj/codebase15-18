...
//    int pageSize;
    ...
    */
    public  synchronized void logWrite(TransactionId tid, Page before,
                                       Page after)
        throws IOException  {
        Debug.log("WRITE, offset = " + raf.getFilePointer()+"tid="+tid.getId());
        preAppend();
    ...
    */
    public synchronized  void logXactionBegin(TransactionId tid)
        ...
        Debug.log("BEGIN OFFSET = " + currentOffset+",tid="+tid.getId());
    }
    ...
    */
    public void rollback(TransactionId tid)
        ...
        synchronized (Database.getBufferPool()) {
            synchronized(this) {
                preAppend();
                long tidL=tid.getId();
                long start=tidToFirstLogRecord.get(tidL);
                raf.seek(start);
                boolean flag=true;
                while(flag){
                	try{
                		int recordType=raf.readInt();//Each log record begins with an integer type
                		long transactionId=raf.readLong();//Each log record begins with a long integer transaction id.
                    	switch(recordType){
                    		case(UPDATE_RECORD):{
                    			//transactionId=raf.readLong();
                    			Page before=readPageData(raf);
                            	Page after=readPageData(raf);
                            	if(transactionId==tidL){
                            		Database.getBufferPool().discardPage(after.getId());//discard the current page
                    	...
                    	raf.readLong();//each record ends with a long file offset representing the position in the log file where the record begins
                        /*
                         * no need to print it out, as there is a if(transactionId==tidL) in each branch
                    	System.out.println("start="+start+",end="+end);
                    	if(tidL!=transactionId){
                        	System.out.println("Something goes wrong, transaction id is different from the parameter transaction id,recordType is "+recordType+",transactionId in file is "+transactionId+",parameter tid is "+tidL);
                        	return;
                        }
                        */
                	}catch(Exception e){
    ...
    public void rollback(long xid,long offset)
            throws NoSuchElementException, IOException {
            synchronized (Database.getBufferPool()) {
                synchronized(this) {
                    preAppend();
    ...
    */
	public void recover() throws IOException {
		synchronized (Database.getBufferPool()) {
			synchronized (this) {
				recoveryUndecided = false;
				raf.seek(0);
				long lcp = raf.readLong();
				Map<Long, Long> map = new HashMap<Long, Long>();
				//if (lcp == -1) {//no matter whether there is any checkpoint or not, it is better to check from start as sometimes, check point doesn't include all the transactions
					while (true) {
						try {
							int type = raf.readInt();
							long xid = raf.readLong();
							...
							case (BEGIN_RECORD): {
								map.put(xid, raf.getFilePointer() - LONG_SIZE
										- INT_SIZE);
								break;
							}
							}
							raf.readLong(); // records ends with offset
						} catch (Exception e) {
							break;
						}
					}
					...
					raf.seek(lcp);
					int ty = raf.readInt();// read the type
					long ttid = raf.readLong();// read the transaction id
					int transNo = raf.readInt();
					System.out.println("type=" + ty + ",tid=" + ttid
					...
					//removed the finished transactions
					for(long tid:removedList)
						map.remove(tid);
    ...
    public void print() throws IOException {
    	...
    	raf.seek(0);
        long lastCheckOffset=raf.readLong();
        sb.append(lastCheckOffset).append("\n");
        long record_tid = raf.readLong();
        int type=raf.readInt();
        ...
        case UPDATE_RECORD:
            ...
            Iterator<Tuple> ite=before.iterator();
            while(ite.hasNext())
